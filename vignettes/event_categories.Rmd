...
If however we may want to determine the categories of event within a set of time series that cross over the equator, we may set the argument `lat_col = TRUE` to allow `category()` to automagically detect the latitude of the events by searching for columns named either 'lat' or 'latitude. See the vignette for [detecting events in gridded data](https://robwschlegel.github.io/heatwaveR/articles/gridded_event_detection.html) for more examples of running **`heatwaveR`** code on gridded data.
...
# Calculate MHW categories using purrr
MHW_cat_ALL <- ts_ALL |> 
  group_by(site) |> 
  # nest() |> 
  # mutate(clim = purrr::map(data ~ts2clm(.x, climatology_period = c("1982-01-01", "2011-12-31"))))
  group_modify(~ {
    ts2clm(.x, climatologyPeriod = c("1982-01-01", "2011-12-31"))
    })
