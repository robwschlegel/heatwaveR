...
test1 <- data.frame(lon_lat_NZ_tb,
                    t = file_date,
                    temp = raster::extract(OISST_raster, lon_lat_NZ_tb[c("lon", "lat")], method = "simple"))
)

data.frame(grid_MUR(bbox_wide_from_name(site_abv)), 
           t = file_date,
           temp = as.vector(raster::extract(MUR_raster, extent_MUR(bbox_wide_from_name(site_abv)), method = "simple")-273.15))
