...
author: "Francois Thoral and Robert Schlegel"
date: "`r Sys.Date()`"
...
output: 
  rmarkdown::html_vignette:
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Trend and Breakpoint Analyses}
...
In the previous vignettes we saw how to [download and prepare OISST data](https://robwschlegel.github.io/heatwaveR/articles/OISST_preparation.html)and to [detect MHWs in gridded data](https://robwschlegel.github.io/heatwaveR/articles/gridded_event_detection.html). In this vignette we will use data subsetted around the New Zealand (NZ) Exclusive Economic Zone (EEZ) for our example to calculate trends and breakpoints for bioregions (from @spalding2007) and seasons as shown in @thoral2022.
...
![MEOW bioregions around New Zealand](MapEcoregions.png){width=500px}

Once we have our MEOW data for NZ ready it's time to find out which pixels specificaly we will want to download.
...
# NB: Should be 5,921 pixels
# 6,398
lon_lat_NZ_tb <- lon_lat_NZ %>% 
...
We begin by loading daily OISST v2.1 data for all pixels around New Zealand (lat 25-55°S, lon 160-190°E) and use the functions `heatwaveR::ts2clim()` and `heatwaveR::detect_event()` to get the climatology from the time series. Note that this full process will take roughly one hour on a desktop computer given the number of pixels and days.

```{r load-data, eval=F}
library(raster)
OISST_url <- "https://www.ncei.noaa.gov/thredds/dodsC/OisstBase/NetCDF/V2.1/AVHRR/198201/oisst-avhrr-v02r01.19820101.nc"
...
system.time(
test1 <- data.frame(lon_lat_NZ_tb,
                    t = file_date,
                    temp = as.vector(raster::extract(OISST_raster, lon_lat_NZ_tb[c("lon", "lat")], method = "simple")))
)

ggplot(data = test1, aes(x = lon, y = lat)) +
...
system.time(
  OISST_data2 <- plyr::ldply(seq(as.Date("1982-01-01"), as.Date("1982-12-31"), by = "day"),
                             OISST_pix_dl, pixel_coords = lon_lat_NZ_tb, .parallel = T)
) # 152 seconds for 1 month
...
# NB: Contains 207,608,896 rows, 4 columns, and uses ~10 GB of RAM
system.time(
  OISST_data3 <- OISST_sub_dl(time_df = data.frame(start = "1982-01-01", end = "1982-12-31"))
) # 
OISST_data <- dl_years %>% 
    group_by(date_index) %>% 
    group_modify(~OISST_sub_dl_NZ(.x)) %>% 
    ungroup() %>% 
    dplyr::select(lon, lat, t, temp)
...
# NB: Takes ~46 minutes on a desktop computer
OISST_clim <- OISST_data %>%
  group_by(lon,lat) %>%
...
We first summarise the MHW metrics by year and season.

```{r summarize-metrics, eval=F}
...
Then we assign bioregions using the `sf` (@sf) package.

```{r assign-bioregion, eval=F}
# We only get the pixels here (rather than time series of MHW days), converted to sf object
# NB: Should be 14,212 pixels, but some out of MEOW polygons
MHW_pixels <- MHW_summary %>% 
  group_by(lon, lat) %>% 
  tally() %>% 
  st_as_sf(coords = c('lon','lat'), crs = 4326) 

# Join the MHW gridded pixels to the MEOW spatial data
MHW_pixels_MEOW <- st_join(MHW_pixels, MEOW_NZ) %>% 
  dplyr::select(geometry, ECOREGION, PROVINCE)

# Filter out pixels not within the NZ EEZ and convert the coordinates back to a tibble for further use
# NB: Should be 5,921 pixels
MHW_pixels_NZ <- MHW_pixels_MEOW %>% 
  dplyr::filter(!is.na(ECOREGION)) %>% 
  mutate(lon = sf::st_coordinates(.)[,1],
         lat = sf::st_coordinates(.)[,2]) %>% 
  as_tibble() %>% 
...
# Filter out pixels not within NZ MEOW
# NB: Should be 2,128,210 rows
MHW_summary_NZ <- left_join(MHW_summary, MHW_pixels_NZ, by = c('lon','lat'))  %>% 
  dplyr::filter(!is.na(ECOREGION))

# Save for future use
saveRDS(MHW_summary_NZ,"~/Desktop/MHW_summary_NZ_1982_2021_OISST.Rds")
```
