...
fFun <- function(data) {
  require(fda)
  b7 <- create.fourier.basis(rangeval = range(data$doy), nbasis = 11)
...
plt3 <- ggplot(data = sst.repl.smth, aes(x = doy, y = smoothed)) +
  ...
  geom_line(aes(group = rep), alpha = 0.3, size = 0.1) +
  facet_wrap(~ts.len, nrow = 3)
ggsave(plt3, file = "rand_mat_smoothed_1b.png", width = 12, height = 6)
```
...
sample_10 <- sst.repl2 %>% 
  ...
  group_by(id, rep) %>% 
  nest() %>% 
  mutate(smoothed = map(data, function(x) ts2clm(x, clmStart = "1983-01-01", clmEnd = "1992-12-31"))) %>% 
  unnest(smoothed)
...
sample_20 <- sst.repl2 %>% 
  ...
  group_by(id, rep) %>% 
  nest() %>% 
  mutate(smoothed = map(data, function(x) ts2clm(x, clmStart = "1983-01-01", clmEnd = "2002-12-31"))) %>% 
  unnest(smoothed)
...
sample_30 <- sst.repl2 %>% 
  ...
  group_by(id, rep) %>% 
  nest() %>% 
  mutate(smoothed = map(data, function(x) ts2clm(x, clmStart = "1983-01-01", clmEnd = "2012-12-31"))) %>% 
  unnest(smoothed)
...
plt4 <- sst.repl2.smth %>% 
  ...
  geom_line(data = filter(sst.repl2.smth, t <= "1984-01-01"), aes(, y = seas_clim_year, group = rep), alpha = 0.3, size = 0.1) +
  facet_wrap(~id, nrow = 3)
ggsave(plt4, file = "rand_mat_smoothed_2.png", width = 12, height = 6)
```
