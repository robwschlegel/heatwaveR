...
res.1.F <- heatwaveR::detect(ts_dat, climatology_start = "1983-01-01",
              ...
              window_half_width = 0, smooth_percentile = F)
...
ggplot(data = dplyr::filter(res.11.F$clim, t <= "1982-12-31"),
  ...
  geom_line(data = dplyr::filter(res.61.F$clim, t <= "1982-12-31"),
            ...
            colour = "red",
            size = 1.2, alpha = 0.4) +
    geom_line(data = dplyr::filter(res.1.F$clim, t <= "1982-12-31"),
            aes(x = t, y = seas_clim_year),
            colour = "green4",
            size = 0.4, alpha = 0.8) +
...
temp_k7 <- create.fourier.basis(rangeval = range(res.1.F$clim$doy), nbasis = 7)
phimat <- eval.basis(res.1.F$clim$doy, basisobj = temp_k7)
phi.frame <- data.frame(cbind(phimat, res.1.F$clim$doy))
melt.phi <- melt(data = phi.frame, id.vars = "V8")
```

```{r}
ggplot(data = melt.phi, aes(x = V8, y = value)) +
...
clm <- res.1.F$clim %>% 
  filter(t <= "1982-12-31")
basis6 <- create.fourier.basis(rangeval = range(clm$doy), nbasis = 6)
fourier6.fd <- smooth.basis(argvals = clm$doy, y = clm$seas_clim_year, fdParobj = basis6)$fd
temp6 <- eval.fd(clm$doy, fourier6.fd)
temps <- setNames(data.frame(clm$doy, clm$seas_clim_year, temp6), c("doy", "climatology", "Fourier_6"))

temps %>% 
  gather(key = "type", value = "temperature", -doy) %>% 
  ggplot(aes(x = doy, y = temperature)) +
  geom_line(aes(colour = type)) +
  geom_line(data = dplyr::filter(res.11.T$clim, t <= "1982-12-31"),
       aes(x = doy, y = seas_clim_year), colour = "black")
```
