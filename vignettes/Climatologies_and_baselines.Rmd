...
plt1 <- ggplot(data = no_smooth,
       aes(x = doy, y = seas_clim_year)) +
  geom_line(aes(colour = width, size = width)) +
  geom_line(aes(y = thresh_clim_year, colour = width, size = width)) +
...
![**Figure 2.** Daily climatologies of the mean (lower lines) and 90th percentile (top lines) produced by the `detect()` function, with various `window_half_width` values and with `smooth_percentile` switched off.](climat_1.png){width=80%}
...
![**Figure 3.** Daily climatologies of the mean (lower lines) and 90th percentile (upper lines) produced by the `detect()` function, with various `window_half_width` values and with `smooth_percentile` at the default value of 31 days.](climat_2.png){width=80%}
...
b7.seas.smth <- smooth.basis(argvals = clm$doy, y = clm$seas_clim_year, fdParobj = b7)$fd
clm.seas.smth <- eval.fd(clm$doy, b7.seas.smth)

# create  smooth threshold
b7.thresh.smth <- smooth.basis(argvals = clm$doy, y = clm$thresh_clim_year, fdParobj = b7)$fd
clm.thresh.smth <- eval.fd(clm$doy, b7.thresh.smth)

temps <- as_tibble(data.frame(doy = clm$doy,
                              seas.unsmth_1 = clm$seas_clim_year,
                              seas.smth_11 = res.11.T$seas_clim_year,
                              clim.fourier_7 = clm.seas.smth,
                              thresh.unsmth_1 = clm$thresh_clim_year,
                              thresh.smth_11 = res.11.T$thresh_clim_year,
