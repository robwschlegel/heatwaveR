---
title: "Comparing the calculation of MHWs in R vs Python - Basic"
...
#print(type(mhws))
#np.savetxt('mhws_py.csv', mhws)
#print(mhws)
#np.save('clim.npy', clim) 
#np.save('event.npy', event) 

#import csv
#with open('mhw_py.csv','wb') as f:
#    w = csv.writer(f)
#    w.writerows(mhws.items())
...
#df = pd.DataFrame([(k, v) for k, v in mhws.iteritems()], columns=['key', 'value'])
# Store the data into a csv file
#df.to_csv('mhw_py.csv', sep=',') # indicate the path where to store the csv file
```
...
# Or just slam everything together
# colnames(res_event_R) <- paste("R", colnames(res_event_R), sep = "_")
# colnames(res_event_Python) <- paste("Py", colnames(res_event_Python), sep = "_")
# res_event_num <- cbind(res_event_R, res_event_Python) %>% 
#   select(-Py_category, -(Py_time_end:Py_time_start), -Py_n_events,
#          -(R_date_start:R_date_peak), -(Py_date_end:Py_date_start),
#          -R_event_no, -(Py_duration_extreme:Py_duration_strong))
# cor(res_event_num)
```
