...
>>>>>>> 570b5edf62cb399f92feb8ed5e827de5ba9247fe
...
With our wrapper function written we would now need to run it several times in order to grab all of the available OISST data from `1982-01-01` to `2018-12-31`. Even though each year of data for the extent used in this vignette is only ~1.0 MB, the server does not like it when more than 9 years of consecutive data are requested. The server will also end a users connection after ~17 individual files have been requested. Because we can't download all of the dat in one request, and we can't download the data one year at a time, we will need to make requests for multiple batches of data. To accomplish this we will create a dataframe of start and end dates that will allow us to automate the entire download while meeting the aforementioned criteria.
