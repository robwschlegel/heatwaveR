...
make_whole <- function(data, x = t, y = temp) {
  ...
  t_series <- zoo::zoo(ts.y, ts.x)
  ts.x.ser <- seq(stats::start(t_series), stats::end(t_series), by = "1 day")
  ts.x.ser <- zoo::zoo(rep(NA, length(ts.x.ser)), order.by = ts.x.ser)
  t_series <- merge(ts.x.ser, t_series)[, 2]

  feb28 <- 59
  v_date <- as.Date(as.POSIXct(t_series))
